...
// 5. project -> properties -> c/c++ -> additional include directiories -> 
//    -> dodajemo putanju gde je mpi sdk include folder
// 6. project -> properties -> linker -> additional library directories ->
//    -> dodajemo putanju do mpi sdk Lib\x86 odnosno Lib\x64 foldera
// 7. project -> properties -> Linker -> Input -> Additional dependencies ->
//    -> ukucamo msmpi.lib
// 8. apply & build proj
// 9. otvaramo terminal i pozicioniramo se u debug folder.
//    Komanda za pokretanje je mpiexec - n 5 MPI-Zadaci.exe
//    Za program iz primera n = 5 procesa
...
{
   // Napisati MPI program koji realizuje množenje matrice A dimenzija n x n i
   // matrice B dimenzija n x n, čime se dobija rezultujuća matrica C. Program
   // pronalazi i prikazuje maksimalni element u rezultujućoj matrici, kao i
   // identifikator procesa koji ga sadrži. Matrice A i B se inicijalizuju u
   // master procesu. Matrica A je podeljena na blokove od po k vrsta (k je
   // zadata konstanta i n je deljivo sa k), a matrica B podeljena u blokove od
   // po k kolona. Master proces distribuira odgovarajuće blokove matrica A i
   // odgovarajuće blokove matrice B procesima radnicima. Ukupan broj procesa
   // je (n/k) ^ 2. Svaki proces obavlja odgovarajuća izračunavanja i učestvuje
   // u generisanju rezultata koji se prikazuje u master procesu. Predvideti da
   // se slanje blokova od po k vrsta matrice A i od po k kolona matrice B
   // svakom procesu odvija sa po jednom naredbom MPI_Send kojom se šalje samo
   // jedan izvedeni tip podatka. Za ostatak izračunavanja koristiti grupne
   // operacije.
   ...
   // Napomena: predviđeno je da se zadatak pokreće sa nProcs = 25!
    ...
    struct MaxValue
    {                           // struktura koju koristimo za pronalazak maksimalne vrednosti
    ...
    // 1. prikupljamo lokalne rezultate mnozenja u prvi red, koristeći
    // komunikatore kolona
    // 2. sada se u svakom procesu iz prvog reda nalazi blok od k kolona
    // preko komunikatora reda skupljamo sve kolone u master proces
    // Napomena: u prvom koraku za prikupljanje lokalnih rezultata u prvi red
    // ponovo koristimo matricu localB zato što je odgovarajućih dimenzija
    ...
    // napomena: postoji i drugi način da se ovo odradi tako što master procesu
    // šaljemo samo max element i onda broadcastujemo taj max element svim
    // procesima -> ako proces sadrži globalni masksimalni element ispisuje rank
    MPI_Reduce(&localMaxValue, &globalMaxValue, 1, MPI_DOUBLE_INT, MPI_MAXLOC , 0, MPI_COMM_WORLD);

    // rezultati
    // ispisujemo rezultujuću matricu na kraju množenja zatim
    // ispisujemo maksimalni element i id procesa koji ga sadrzi
    if (rank == 0)
    {
...
{
    // Napisati MPI program kojim se podaci o nizu studenata koji se unose sa
    // tastature u master procesu, prosleđuju odjednom svim procesima u 
    // komunikatoru MPI_COMM_WORLD. Za svakog studenta se pamte: broj indeksa
    // (int), ime (string), prezime (string), prosečna ocena (float) i godina
    // studija (int). Zadatak rešiti korišćenjem grupnih operacija i izvedenih
    // tipova podataka.
    ...
    // Napomena: isto se može postići i sa MPI_Address ali je komplikovanije
    ...
    // master proces je zadužen za unos studenata i slanje ostalim procesima
    // dok ostali procesi samo primaju podatke
    if (rank == 0)
    {
        for (int i = 0; i < N; i++)
        {
    ...
    // radi provere ispravnosti programa uzimamo da jedan proces ispisuje
    // primljene podatke
    if (rank == 1)
    {
